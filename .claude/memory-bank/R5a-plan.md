...
### ✅ R5a-2 — ManualAsset 부동산·연금 (완료, 미커밋)

> **스코프 변경**: 평가이력 테이블(manual_asset_valuations)은 **안 만듦** — `manual_assets` 단일(current_valuation + valued_at)만. as-of/carry-forward는 R5a-3로 이동.
> **구현**: 백 `manual_asset` 도메인 5파일 + AccountType(REAL_ESTATE/PENSION) + `_calc_balance` roll-up 분기 + `_build_allocation` **account_type→asset_class 매핑**(전용계좌 balance를 슬라이스로 — ManualAsset 따로 합산 X, double-counting 불가) + 마이그레이션 `3ef71953bb00`. 프론트 `_features/manual-asset/` 7파일(폼은 modal) + wealth-section 섹션 + i18n.
> **검증**: 가역성/typecheck/lint/curl 모두 통과(부동산 5억→총자산 정확히 증가 + REAL_ESTATE 슬라이스).
...
> **설계 변경**: 원안(전용 `asset_class_snapshots` 테이블 + 새 도메인 + 박제 통합점) **폐기** → **방안 B 경량** 채택. 새 도메인/테이블 0개.
> **이유**: ① R5a-2 roll-up 덕분에 부동산/연금 과거 as-of = AccountSnapshot의 REAL_ESTATE/PENSION 계좌 balance(매달 박제) → ManualAsset 평가이력 테이블 불필요. ② INVESTMENT 과거 cash = `AccountSnapshot.balance − Σ PVH.valuation`(`_calc_balance` 역산). ③ PVH에 asset_class만 박으면 codex "aggregation loss"도 해소.
>
> **구현(방안 B)**:
> - 백: `PortfolioValueHistory.asset_class` 컬럼 추가 + 마이그레이션 `c9d4e7f21a36`(기존 행 portfolio_items.asset_class로 백필 UPDATE). `snapshot_service.py` 박제 시 `asset_class=i.asset_class` 기록. `PortfolioValueHistoryRepository.find_by_household_and_range` 신규. `wealth/service.py`에 `build_allocation_trend(account_snapshots, portfolio_histories, accounts)` 순수 집계(`_slices_to_list` 공통 추출) + `get_wealth_overview` 통합(`find_by_ids`로 삭제계좌 포함). `resolve_snapshot_range` 헬퍼를 account_snapshot/service에서 추출해 yearly와 축 공유. schema `AllocationTrendPoint` + `AllocationResponse.allocation_trend`.
> - 프론트: `allocation-trend-chart.tsx`(useMounted SSR 가드 + 스택 `<Area stackId="allocation">` + 없는 슬라이스 0패딩 + `ASSET_CLASS_COLOR`). `wealth/types.ts` `AllocationTrendPoint`. wealth-section 배분 도넛 카드 직후 부착. **api.ts/i18n 수정 불필요**(allocation은 타입 참조로 자동 매핑, enum 라벨은 기존 `enum.asset-class` 재사용, 카드 타이틀은 기존 하드코딩 관행).
>
> **검증 통과**: 마이그레이션 가역성 왕복 · curl(모든 월 `trend total == yearly totalBalance`, ratio합 100, cash 역산 정확, 이중계상 0, 수동박제 후 5월 부동산6.2억/연금4800만 반영) · E2E 렌더(`/browse`로 5개 자산군 스택영역·색상·툴팁 확인, height 200·area path 정상).

<details><summary>원래 계획 (폐기 — 참고)</summary>
...
## 작업 순서
✅ R5a-1(64d7571) → ✅ R5a-2(94452dd) → ✅ R5a-3(방안 B 경량). **R5a 사이클 전부 완료**. 다음: dev→main 머지 검토.
